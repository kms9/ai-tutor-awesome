...
| 维度 | OpenTutor | Course Navigator | ClassroomIO | StudyFetch | YouLearn | Scholarly | Coursebox | X-Pilot | NotebookLM | Khanmigo | Brilliant |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 资料摄入 | 强（PDF/Office/Canvas） | 视频/字幕 | 课包+视频转写 | 课件/音视频/手写 | PDF/YT/录音 | PDF/录音/笔记 | 文档/视频/URL | PDF/PPT/MD/URL | 笔记本源 | Khan 内容库 | —（官方课） |
| 一键成课 | 中（笔记+测验包） | — | 中（AI 大纲/讲义） | 弱（复习包） | 中（笔记+测+播客） | 中（积分创作） | 强（培训课） | 强（视频课） | 弱（指南/概览） | — | —（手写关卡） |
| 直播课堂 | — | — | — | 转写助手 | — | — | — | — | — | — | — |
| 解题/出题 | 强（7 题型+FSRS） | — | 练习+辅助批改 | 测验 | 测验/模考 | 测验/模考 | 课内测验 | — | 学习指南 | 站内练习+追问 | 强（交互关卡+Koji） |
| 主动规划 | 中（日历） | — | 合规截止日期 | 弱 | 弱 | 弱 | — | — | — | — | 中（Path/掌握度） |
| 长期记忆 | 工作区状态 | — | LMS 进度 | 弱 | 错题聚焦 | 工作区 | LMS 进度 | 项目文件 | 笔记本 | 账号历史 | 掌握记录/仪表盘 |
| 视觉知识结构 | 知识图谱（实验） | — | — | — | — | 思维导图/幻灯 | 数字人口播 | 分镜视频 | 幻灯/视频概览 | — | 极强（课内画布） |
| 可复用 Skill | 未见 | 无 | MCP，非教学 Skill | 无 | 无 | 无 | 无 | 无 | 无 | 无公开文件 | 无（Koji 锁产品内） |
| 自托管 / 换模型 | 是 | 本地 App | 是 | 否 | 否 | 聊天可选模型 | 否 | 否 | 否 | 否 | 否 |
| 导出 | 会话/复习内容 | 字幕/文本 | SCORM 弱于 Coursebox | 复习集 | Anki（Pro） | Word/PPT/Anki（付费） | PDF/HTML/SCORM | MP4 | 音频下载 | — | 账号进度；无证书 |
